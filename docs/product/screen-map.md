...
**Actions**: answer → autosave to `exam_attempts.answers`; "Nộp bài" → confirm dialog → `grade-exam` → `/mock-test/result/:attemptId`
**Special**: timer ticks via `Ticker`; section transition card shown between skills
...
**States**: `loading` (shimmer), `success`, `error`
**UI**: total score hero, pass/fail badge, skill breakdown chart, weak skills list, CTA (signup if guest, retry or review if auth)
